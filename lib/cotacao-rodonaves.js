...
const cadastrarClienteRodonaves = require('./rodonaves-cadastrar-cliente');
...
module.exports = async (req, res) => {
  ...
  // Sonda da malha de atendimento (24/09/2026). NÃO bloqueia nada e não é
  // chamada pela cotação — existe só para descobrir o formato da resposta.
  //
  // Por que: hoje a única forma de saber que a Rodonaves não atende uma
  // cidade é cotar, esperar até 20s e ler a recusa ("Não localizada uma
  // unidade que atenda a cidade: 1516 - MOMBACA"). A doc lista um endpoint
  // de malha — busca-por-cep, que devolve a unidade que atende um CEP — e
  // ele mora no MESMO domínio do prazo (01wapi), apesar de o portal chamar
  // a seção de "unittocity". Se ele disser "nenhuma unidade" de forma
  // confiável, dá para saber antes de cotar, sem manter regra nenhuma.
  //
  // A doc não mostra o corpo da resposta de sucesso. Então o primeiro passo
  // é olhar uma resposta real: esta ação devolve o corpo CRU, sem
  // interpretar. Só depois de ver um CEP atendido e um não atendido é que
  // vale escrever qualquer lógica em cima disso.
  if ((req.body || {}).acao === 'malha') {
    const cep = String((req.body || {}).cep || '').replace(/\D/g, '');
    if (cep.length !== 8) {
      res.status(200).json({ erro: true, mensagem: 'Informe um CEP com 8 dígitos.' });
      return;
    }
    try {
      const token = await getToken(DOMINIO_PRAZO);
      const url = `${DOMINIO_PRAZO}/api/v1/busca-por-cep?zipCode=${cep}`;
      const resp = await fetch(url, {
        headers: { Authorization: `Bearer ${token}`, accept: 'application/json' },
      });
      const texto = await resp.text();
      console.log('[rodonaves-malha]', JSON.stringify({ cep, status: resp.status, resposta: texto.slice(0, 2000) }));
      res.status(200).json({ ok: true, cep, status: resp.status, corpo: texto.slice(0, 4000) });
    } catch (err) {
      res.status(200).json({ erro: true, mensagem: err.message || 'Erro ao consultar a malha da Rodonaves.' });
    }
    return;
  }
